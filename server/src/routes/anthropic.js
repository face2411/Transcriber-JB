...
anthropicRouter.post("/fast-research", handler(async (req, res) => {
  ...
  const search = await completeWithWebSearch({
    ...
    prompt: `Research "${company}"${url ? ` (website: ${url})` : ""}. Find: what they do, their approximate employee count and revenue scale, whether this is genuinely their primary business or a division of a larger/different-industry parent, industry vertical, recent news (executive hires, M&A, funding, expansion, technology investments, partnerships), and specifically whether they already operate a captive/offshore IT or software delivery center (India GCC or otherwise) or are owned by a parent that does. State plainly if you searched for size or GCC/offshore evidence and found nothing, rather than omitting it. Plain text summary only.`,
    maxTokens: 2000,
  });
...
SIZE AND GCC ARE HARD GATES: if the findings show this company is meaningfully outside the target size band, is a minor division of a larger/different-industry parent, or shows any evidence of offshore/GCC IT delivery, that must appear as a flag and must cap "qualification" at "Flags Present" or "Disqualified" - a strong signal elsewhere does not offset a real size or GCC disqualifier. If the findings explicitly found no evidence either way on size or GCC, say so in qualification_rationale rather than treating silence as a clean bill of health.
...
SIZE/INDUSTRY FIT IS A FILTER, NOT A CAVEAT - if you are not reasonably confident a company both (a) has this vertical as its actual primary business, not a minor division or adjacent market, and (b) is roughly within the ${sizeRange} band, LEAVE IT OUT of the list entirely rather than including it with a hedge like "likely" or "approximately." A well-known Fortune 500 / large-cap company, or a company you know to be a small niche shop under ~100 employees, does not belong on this list even if it operates in the vertical - do not include it "for completeness." It is better to return 8 confident candidates than 12 where several are size or industry mismatches.

GCC/OFFSHORE RISK - DEFAULT TO CAUTION, NOT OPTIMISM: for any company you know (or strongly suspect from its scale/profile) to be a large-cap, Fortune 1000, or well-known multinational, treat gcc_risk as at least "medium" by default even without a specific confirmed fact - large companies commonly run captive offshore/India delivery centers, and defaulting to "low" risk for a company you have no real basis to clear is how disqualifying companies slip through. Only mark gcc_risk "low" when the company's small-to-mid scale genuinely makes an in-house offshore captive unlikely, or a general contractor whose business is not IT and clearly not run offshore.
...
anthropicRouter.post("/news-intel", handler(async (req, res) => {
  ...
  const search = await completeWithWebSearch({
    ...
    prompt: `Find recent news about "${name}"${urlContext ? ` (${urlContext})` : ""} - leadership changes, M&A, expansions, technology investments, partnerships, construction projects, events. Also specifically search for and report on these two disqualifying-risk questions, since they're the most common way a candidate turns out to be a bad fit even when the news looks good:
1. Company size/scale - actual employee count or revenue if findable, and whether this company's primary business genuinely is the vertical it was listed under (not a minor division of a much larger/different-industry parent).
2. Offshore/GCC delivery - any evidence this company already operates a captive/offshore IT or software delivery center (India GCC or otherwise), or is a subsidiary of a parent company that does.
Also find their exact corporate headquarters city and state. ${anchorUrl ? `Only include results that are clearly about this specific company at ${anchorUrl}.` : ""} Write a plain text summary of everything you find, including explicitly stating if you found nothing on the size or GCC questions - that absence matters as much as a positive finding.`,
    maxTokens: 2000,
  });
...
SIZE AND GCC ARE HARD GATES, NOT SOFT SIGNALS: if the findings above indicate this company is meaningfully outside the target size band, is a minor division of a larger/different-industry parent, or shows any evidence of offshore/GCC IT delivery, that must produce a "flag" entry stating it plainly and must pull signal_tier down (never Tier 1 with an unresolved size or GCC concern) - do not let a good news signal outweigh a real disqualifier. If the findings explicitly found NO evidence either way on size or GCC (searched and came up empty), say so in tier_rationale rather than silently treating absence of evidence as a clean bill of health.
